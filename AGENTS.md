...
- Game reference data (`data/kanji-components.min.json`) is generated by `scripts/build_components.py`
  from KRADFILE-u and committed. It is invariant-tested but NOT part of the markdown drift check
  (`git diff --exit-code -- data/`); do not hand-edit it either.
- Compound data is markdown-sourced: edit `content/compounds/*.md`, run `python3 scripts/compile_content.py`,
  and commit `data/compounds.min.json`. `scripts/build_compounds.py` only imports new JMdict words
  into the markdown and never overwrites existing entries.
